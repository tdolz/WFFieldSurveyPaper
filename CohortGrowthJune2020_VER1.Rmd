...
**The multi-histogram function**
could be useful later, but for now we are not using this. 
```{r,eval=FALSE}
#week3
chrt_wk(split_df$`3`$value,30,50,2,2,0.8659636,	0.1340364)#15
#wk3.16 <- as.data.frame(split_df$`3`) %>% mutate(co= 1) #NO SPLIT
wk3.16 <- as.data.frame(split_df$`3`) %>% mutate(co=ifelse(value <= 47,2,1)) %>% mutate(co=as.factor(co))#SPLIT

plot_multi_histogram <- function(df, feature, label_column,thresh) {
    plt <- ggplot(df, aes(x=eval(parse(text=feature)), fill=eval(parse(text=label_column)))) +
    geom_histogram(alpha=0.7, position="identity", aes(y = ..density..), color="black") +
    geom_density(alpha=0.7) +
    #geom_vline(aes(xintercept=mean(eval(parse(text=feature)))), color="black", linetype="dashed", size=1) +
    geom_vline(aes(xintercept= thresh), color="black", linetype="dashed", size=1)+
    labs(x=feature, y = "Density") + 
    theme_classic()  
    plt + guides(fill=guide_legend(title=label_column))
}

plot_multi_histogram(wk3.16,"value","co", thresh=47)
```

**The multi-histogram function -CUSTOM DIST**
Not working at the moment but that's ok. 
```{r, eval = FALSE}
#week3
chrt_wk(split_df$`3`$value,30,50,2,2,0.8659636,	0.1340364)#15
#wk3.16 <- as.data.frame(split_df$`3`) %>% mutate(co= 1) #NO SPLIT
...
plot_multi_histogram <- function(df, label_column,thresh, mu1, mu2, sig1,sig2) {
    plt <- ggplot(df) +
    geom_histogram(alpha=0.7, position="identity", aes(y = ..density..,x=label_column), color="black") +
     #geom_histogram(aes(x=label_column),alpha=0.7, position="identity", color="black") +
    stat_function(aes(x = label_column),fun=dnorm, color="red", args=list(mean=mu1,sd=sig1))+
    stat_function(aes(x = label_column),fun=dnorm, color ="green", args=list(mean=mu2,sd=sig2))+
    geom_vline(aes(xintercept= thresh), color="black", linetype="dashed", size=1)+
...
plot_multi_histogram(wk3.16,"value",thresh=47, mu1=36.0203096,mu2=39.1064080,sig1=4.0636209,sig2=11.9268363)
```

**Split Lyndie's Cohorts: 2010**
We are not doing this one because there were more weeks where "no split" was reccomended than there were where split was reccomended. 
```{r, eval=FALSE}
split_df <- allmelt %>%
  filter(Bay == "Shinnecock")%>% filter(Year=="2010")%>%
  split(.$Week) 
...
#week1
chrt_wk(split_df$`1`$value,30,45,3,3,0.3659931,0.6340069,5)
#wk1.10 <- as.data.frame(split_df$`1`) %>% mutate(co= ifelse(value <= 36,2,1)) 
wk1.10 <- as.data.frame(split_df$`1`) %>% mutate(co= 1) #NO SPLIT
#week2
chrt_wk(split_df$`2`$value,35,55,4,4.4,0.4997561,	0.5002439,5)
wk2.10 <- as.data.frame(split_df$`2`) %>% mutate(co= ifelse(value <= 43,2,1)) 
#week5
chrt_wk(split_df$`5`$value,45,70,4,4,0.2847989,	0.7152011,10)
wk5.10 <- as.data.frame(split_df$`5`) %>% mutate(co= ifelse(value <= 49,2,1))
#week7
chrt_wk(split_df$`7`$value,50,65,3,3,0.90939024,	0.09060976,20)
wk7.10 <- as.data.frame(split_df$`7`) %>% mutate(co=1) #NO SPLIT
#wk7.10 <- as.data.frame(split_df$`7`) %>% mutate(co=ifelse(value <= 74,2,1)) #SPLIT
#week9
chrt_wk(split_df$`9`$value,60,90,5,3,0.92619298,	0.07380702,10)
wk9.10 <- as.data.frame(split_df$`9`) %>% mutate(co=1) #NO SPLIT
#wk9.10 <- as.data.frame(split_df$`9`) %>% mutate(co=ifelse(value <= 80,2,1)) 
#week12
chrt_wk(split_df$`12`$value,60,80,2,1,0.4463557,	0.5536443,5)
#wk12.10 <- as.data.frame(split_df$`12`) %>% mutate(co= ifelse(value <= 80,2,1)) #SPLIT
wk12.10 <- as.data.frame(split_df$`12`) %>% mutate(co=2) #NO SPLIT
#week13
chrt_wk(split_df$`13`$value,60,90,2,8,0.4364544,	0.5635456,10)
wk13.10 <- as.data.frame(split_df$`13`) %>% mutate(co= 2) #NO SPLIT
#wk13.10 <- as.data.frame(split_df$`13`) %>% mutate(co=ifelse(value <= 77,2,1)) #SPLIT
#week15
chrt_wk(split_df$`15`$value,70,90,3,3,0.7153474,	0.2846526,5)
wk15.10 <- as.data.frame(split_df$`15`) %>% mutate(co= ifelse(value <= 83,2,1))
#week17
#chrt_wk(split_df$`17`$value,60,100,2,1,5)
wk17.10 <- as.data.frame(split_df$`17`) %>% mutate(co= ifelse(value <= 90,2,1))
#week20
#chrt_wk(split_df$`20`$value,60,90,0.5)
wk20.10 <- as.data.frame(split_df$`20`) %>% mutate(co= 2)

lynco2010 <-bind_rows(wk1.10,wk2.10,wk5.10,wk7.10,wk9.10, wk12.10,wk13.10,wk15.10,wk17.10,wk20.10)

#this is a quick check. You could plot the invidual distributions over each week using 
#https://stackoverflow.com/questions/6957549/overlaying-histograms-with-ggplot2-in-r
#but DO NOT do this unless it's absolutely necessary. 
lynco2010 %>%
ggplot(aes(value)) + 
    geom_histogram(data = subset(lynco2010, co==1), fill = "#a6bddb", alpha = 0.8) + 
    geom_histogram(data = subset(lynco2010, co==2), fill = "#1c9099", alpha = 0.8) +
    facet_wrap(~Day, nrow=10)+
    theme_classic()

```
We are not going to include these splits in the final dataset because more than half of the models advocated that we should not split the data. 

**Now split cohorts in 2011.**
We are not doing this one because there were more weeks where "no split" was reccomended than there were where split was reccomended. 
```{r,eval=FALSE}
split_df <- allmelt %>%
  filter(Bay == "Shinnecock")%>% filter(Year=="2011")%>%
  split(.$Week)
...
# week3
chrt_wk(split_df$`3`$value,35,45,2,2,0.16601,	0.83399)#5
wk3.11 <- as.data.frame(split_df$`3`) %>% mutate(co= 1) 
#week5
chrt_wk(split_df$`5`$value,45,60,4,4,0.6458702,	0.3541298)#10
#wk5.11 <- as.data.frame(split_df$`5`) %>% mutate(co= ifelse(value <= 40,2,1)) #SPLIT
wk5.11 <- as.data.frame(split_df$`5`) %>% mutate(co= 1) # NO SPLIT
#week8
chrt_wk(split_df$`8`$value,40,67,8,9,0.2913258,	0.7086742)#8
wk8.11 <- as.data.frame(split_df$`8`) %>% mutate(co= ifelse(value <= 55,2,1)) #SPLIT
#wk8.11 <- as.data.frame(split_df$`8`) %>% mutate(co= 1) #NO SPLIT
#week10
chrt_wk(split_df$`10`$value,50,80,3,4,0.6917516,	0.3082484,20)
#wk10.11 <- as.data.frame(split_df$`10`) %>% mutate(co= ifelse(value <= 97,2,1)) #SPLIT
wk10.11 <- as.data.frame(split_df$`10`) %>% mutate(co= 2) #no split
#week12
chrt_wk(split_df$`12`$value,60,90,7,7,0.91196261,	0.08803739,10)
#wk12.11 <- as.data.frame(split_df$`12`) %>% mutate(co= ifelse(value <= 97,2,1)) #SPLIT
wk12.11 <- as.data.frame(split_df$`12`) %>% mutate(co= 2) #NO SPLIT
#week14
chrt_wk(split_df$`14`$value,70,110,10,10,0.5449067,	0.4550933,25)
wk14.11 <- as.data.frame(split_df$`14`) %>% mutate(co= ifelse(value <= 93,2,1))
 #week18
chrt_wk(split_df$`18`$value,70,110,10,10,0.5706097,	0.4293903,10)
#wk18.11 <- as.data.frame(split_df$`18`) %>% mutate(co= ifelse(value <= 91,2,1)) #SPLIT
wk18.11 <- as.data.frame(split_df$`18`) %>% mutate(co= 2) #NO SPLIT
#week21
chrt_wk(split_df$`21`$value,80,120,9,9,0.8181976,	0.1818024,15)
wk21.11 <- as.data.frame(split_df$`21`) %>% mutate(co= ifelse(value <= 110,2,1)) #SPLIT
#wk21.11 <- as.data.frame(split_df$`21`) %>% mutate(co= 2) #NO SPLIT
#week24
#chrt_wk(split_df$`24`$value,55,85,2,2, 5)
wk24.11 <- as.data.frame(split_df$`24`) %>% mutate(co= 2)

lynco2011 <-bind_rows(wk3.11,wk5.11,wk8.11,wk10.11,wk12.11,wk14.11,wk18.11,wk21.11,wk24.11)


lynco2011 %>%
ggplot(aes(value)) + 
    geom_histogram(data = subset(lynco2011, co==1), fill = "#a6bddb", alpha = 0.8) + 
    geom_histogram(data = subset(lynco2011, co==2), fill = "#1c9099", alpha = 0.8) +
    facet_wrap(~Day, nrow=10)+
    theme_classic()
```
...
```{r, eval=FALSE}
split_df <- allmelt %>%
  filter(Bay == "Mattituck")%>% filter(Year=="2015")%>%
  split(.$Week) 
...
#week6
chrt_wk(split_df$`6`$value,20,50,1,18,0.1,0.9,70)
wk6.15 <- as.data.frame(split_df$`6`) %>% mutate(co= ifelse(value <= 43,2,1)) 
#week8
chrt_wk(split_df$`8`$value,35,65,2,2,0.4,0.6,100)
wk8.15 <- as.data.frame(split_df$`8`) %>% mutate(co= ifelse(value <= 47,2,1))
#week10
chrt_wk(split_df$`10`$value,30,70,2,6,0.8,0.2,20)
#wk10.15 <- as.data.frame(split_df$`10`) %>% mutate(co= ifelse(value <= 49,2,1)) #SPLIT
wk10.15 <- as.data.frame(split_df$`10`) %>% mutate(co= 1) #NO SPLIT
#week12
chrt_wk(split_df$`12`$value,35,85,2,6,0.8,0.2,20)
wk12.15 <- as.data.frame(split_df$`12`) %>% mutate(co= ifelse(value <= 43,2,1))
#week14
chrt_wk(split_df$`14`$value,45,85,6,6,0.6,0.4,10)
wk14.15 <- as.data.frame(split_df$`14`) %>% mutate(co= ifelse(value <= 57,2,1))
#week16
chrt_wk(split_df$`16`$value,60,100,6,6,0.6,0.4,5)
wk16.15 <- as.data.frame(split_df$`16`) %>% mutate(co= ifelse(value <= 70,2,1))

Mtco2015 <-bind_rows(wk6.15,wk8.15,wk10.15,wk12.15,wk14.15,wk16.15)

Mtco2015 %>%
ggplot(aes(value)) + 
    geom_histogram(data = subset(Mtco2015, co==1), fill = "#a6bddb", alpha = 0.8) + 
    geom_histogram(data = subset(Mtco2015, co==2), fill = "#1c9099", alpha = 0.8) +
    facet_wrap(~Date, nrow=10)+
    theme_classic()

```

**Lets try the other bays?**
Moriches 2010 - from visparams only 3/7 weeks look like you could feasibly split them and that is being generous. 
```{r, eval=FALSE}
split_df <- allmelt %>%
  filter(Bay == "Moriches")%>% filter(Year=="2010")%>%
  split(.$Week) 
...
Moriches 2011 - from visparams only 3/7 weeks look like you could feasibly split them and that is being generous. 
```{r, eval=FALSE}
split_df <- allmelt %>%
  filter(Bay == "Moriches")%>% filter(Year=="2011")%>%
  split(.$Week) 
...
#week3
chrt_wk(split_df$`3`$value,30,85,1,18,0.1,0.9)
wk6.11 <- as.data.frame(split_df$`3`) %>% mutate(co= 1) #no split
#week5
chrt_wk(split_df$`5`$value,35,65,2,2,0.4,0.6)
wk8.11 <- as.data.frame(split_df$`5`) %>% mutate(co= ifelse(value <= 45,2,1)) #split
#week8
chrt_wk(split_df$`8`$value,65,95,2,6,0.8,0.2)
wk10.11 <- as.data.frame(split_df$`8`) %>% mutate(co= 1) #NO SPLIT
#week10
chrt_wk(split_df$`10`$value,35,85,2,6,0.1,0.9) #no split
#week12
chrt_wk(split_df$`12`$value,80,100,6,6,0.6,0.4)
```

#moriches 2016
Low sample size every week means we are not going to try to split these. 
```{r, eval=FALSE}
split_df <- allmelt %>%
  filter(Bay == "Moriches")%>% filter(Year=="2016")%>%
  split(.$Week) 
...
```{r, eval=FALSE}
split_df <- allmelt %>%
  filter(Bay == "Napeague")%>% filter(Year=="2010")%>%
  split(.$Week) 
...
chrt_wk(split_df$`3`$value,25,65,7,7,0.5,0.5) #plit but it doesn't look like you should. 
chrt_wk(split_df$`6`$value,35,75,7,7,0.7,0.3)# split
chrt_wk(split_df$`8`$value,45,65,4,4,0.5,0.5)# no split
chrt_wk(split_df$`10`$value,45,65,9,9,0.2,0.8) #no split
chrt_wk(split_df$`12`$value,40,75,9,9,0.1,0.9) #no split
chrt_wk(split_df$`14`$value,55,80,11,11,0.5,0.5)# no split
chrt_wk(split_df$`16`$value,50,85,9,9,0.5,0.5)#ns
chrt_wk(split_df$`19`$value,60,100,10,10,0.5,0.5)

```

**Napeague 2016**
We're not doing this one sample size is too low. 
```{r, eval=FALSE}
split_df <- allmelt %>%
  filter(Bay == "Napeague")%>% filter(Year=="2016")%>%
  split(.$Week) 
...
```{r, eval=FALSE}
split_df <- allmelt %>%
  filter(Bay == "Jamaica")%>% filter(Year=="2010")%>%
  split(.$Week) 
...
```{r, eval=FALSE}
split_df <- allmelt %>%
  filter(Bay == "Jamaica")%>% filter(Year=="2011")%>%
  split(.$Week) 
...
```{r, eval=FALSE}
split_df <- allmelt %>%
  filter(Bay == "Jamaica")%>% filter(Year=="2016")%>%
  split(.$Week) 
...
```{r, eval=FALSE}
split_df <- allmelt %>%
  filter(Bay == "Cold Spring Pond")%>% filter(Year=="2010")%>%
  split(.$Week) 
...
ggplot(aes(y=value,x=Day))+
  geom_point(aes(y=value,x=Day,fill=co,color=co))+
  scale_x_discrete()+
  geom_smooth(aes(group=co,y=value,x=Day,fill=co, color=co), method="lm", formula= my.formula, se=TRUE, fullrange=FALSE)+
  ...
  theme(axis.text.x = element_text(angle = 90),panel.background = element_rect(fill = "white", colour = "white"))
#ggsave("growthslopecoALL_starred.png", path="/Users/tdolan/Documents/WF SK PROJ/Survey data/Field Survey Paper/final figures")
#dev.off()
...
ggplot(aes(y=value,x=select.day))+
  geom_point(aes(y=value,x=select.day, color=co))+
  #scale_x_discrete()+
  geom_smooth(aes(group=co,y=value,x=select.day,fill=co, color=co), method="lm", formula= my.formula, se=TRUE, fullrange=FALSE)+
  scale_fill_manual(values=cohortcolors)+scale_color_manual(values=cohortcolors)+
  ylab("length (mm)")+ ggtitle("Shinnecock cohorts")+ xlab("Days since selection to the gear")+
  facet_grid(~BayYear,scales="free_x")+
  theme(axis.text.x = element_text(angle = 90),panel.background = element_rect(fill = "white", colour = "white"))
#ggsave("growthslopecoSH_lagged.png", path="/Users/tdolan/Documents/WF SK PROJ/Survey data/Field Survey Paper/final figures")
#dev.off()
